...
export function GradebookTable({ courseId, canWrite }: { courseId: string; canWrite: boolean }) {
  ...
  const showCert = canWrite && !!data?.certificatesEnabled;
  ...
  return (
    <div className="space-y-3">
      {canWrite && !data.certificatesEnabled && (
        <p className="rounded-xl border border-dashed border-gray-200 px-4 py-3 text-sm text-muted-foreground dark:border-gray-800">
          Certificados desativados nesta turma. Ative em{" "}
          <strong>Turmas → Editar</strong> para emitir.
        </p>
      )}
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              {showCert && <TableHead className="text-right">Certificado</TableHead>}
            </TableRow>
          </TableHeader>
          <TableBody>
            {data.rows.map((r) => (
              <TableRow key={r.userId}>
                <TableCell className="sticky left-0 bg-white font-medium dark:bg-gray-900">
                  {r.name}
                </TableCell>
                {data.activities.map((a) => (
